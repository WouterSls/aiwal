---
name: uniswap-api-swap-integration
...
function useSwap() {
  ...
  const executeSwap = async (permit2Signature?: string) => {
    ...
    if (!data.swap?.data || data.swap.data === "" || data.swap.data === "0x") {
      throw new Error(
        "Empty swap data - quote may have expired. Please refresh.",
      );
    }
...
async function executeSwapTransaction(
  ...
  if (!publicClient)
    throw new Error(`No public client configured for chainId ${chainId}`);
